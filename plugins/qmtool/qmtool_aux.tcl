...
###########################################################
# Returns the UNSIGNED dihedral angle between the four points.      #
###########################################################

proc ::QMtool::dihed_angle_unsigned { coord1 coord2 coord3 coord4 } {
...
###########################################################
# Returns the SIGNED dihedral angle between the four points.      #
###########################################################

proc ::QMtool::dihed_angle { coord1 coord2 coord3 coord4 } {
   set v1 [vecsub $coord2 $coord1]
   set v2 [vecsub $coord3 $coord2]
   set v3 [vecsub $coord4 $coord3]
   #if {[veclength [veccross $v2 $v3]]==0} {puts "$coord3; $coord4"; puts "$v2 $v3"}
   set cross1 [veccross $v1 $v2] 
   set cross2 [veccross $v2 $v3] 
   set psin [vecdot [vecscale [veclength $v2] $v1] $cross2]
   set pcos [vecdot $cross1 $cross2]
   set angle [::util::rad2deg [expr atan2($psin,$pcos)]]
   return $angle
}
