...
# be parsed.
#
# $Id: qmtool_readwrite.tcl,v 1.62 2020/11/24 00:58:56 mariano Exp $
#
...
proc ::QMtool::read_zmatrix { fid {termstring {}}} {
   ...
   foreach line $zmatrix {
      ...
      if {[string is alpha [string index $name 0]]} {
         ...
         # FIX BUG FOR 2-letter symbols
         if {[string index $name 1]!="" && [string is alpha [string index $name 1]]} {
            # second char is also a letter
            # Get canonical element symbol
	         regexp {[a-zA-Z][a-zA-Z]} $name element
         } else {
      ...
      #catch for name Ow in fftk water interaction files
      if {$element=="" && $name=="Ow"} {set element O}
...
proc ::QMtool::get_cartesian_from_zmat {cart atomlist intcoor} {
   ...
   # Rotate the dihedral
   # changed order of p1, p2 as axis should be [vecsub $p2 $p1]
   set m [trans bond $p2 $p1 $dihed deg]
...
proc ::QMtool::read_gaussian_log { file {intomolid -1}} {
   ...
   variable orientation {}
    ...
    ####
    # added by CGM
    # required to handle changes in log file formats
    ####
    # find the Gaussian version code
    set Gversion ""
    while { ![eof $fid] } {
        set line [string trimleft [gets $fid]]
        if { [regexp {^\*{10,}$} $line] } {
            set line [string trimleft [gets $fid]]
            set Gversion [lindex [ split [lindex $line 2] "-"] 1]
            break
        } else {
            continue
        }
   ...
   while {![eof $fid]} {
      ...
      # add another case, by ys_song
      if {[string match "Symbolic Z-matrix:" $line] >= 0} {
            set title "no title" 
            set zmatgiven_ys 1
            break 
      }
   ...
   if {[winfo exists .qm_setup.edit.titleentry]} {
    ...
    # Read molecule specifications:
    # NOTE: looking for two consecutive lines that indicate, is ZMAT given? Charge & Multiplicity?
    # this method should be agnostic to G09RevA|B|C|D in which the order of appearance diffs
    # both between G09 revisions and specific types of calculations within a given revision
    set zmatgiven 0; set chargemultgiven 0
    while { ![eof $fid] && (!$zmatgiven || !$chargemultgiven) } {
        # keep reading until we reach the end of file or find all three quantities
        set line [string trim [gets $fid]]

        # Z-matrix given explicitly?
        # note subtle differences in Gaussian keywords between G09 revisions
        # add by ys_song
        if {[string match "Z-Matrix taken from the checkpoint file:" $line] ||
            ...
            [string first "Redundant internal coordinates found in file." $line] || 
            [zmatgiven_ys] >= 0} {
            set zmatgiven 1 
        }
...
#      
#        # G09 Rev C and later
#        if { [scan [string index $Gversion 6] %c] >= 67 } {
#            # charge/multiplicity comes first
#            if {[lindex $line 0]=="Charge" && [lindex $line 3]=="Multiplicity"} {
#               variable totalcharge 0
#               variable multiplicity 0
#               set totalcharge [lindex $line 2]
#               set multiplicity [lindex $line 5]
#            }
#            # explicit z-matrix tag/flag comes second
#            # NOTE: this has been tested for:
#            #       -- loading log file from geometry optimization
#            #       -- loading log file from hessian calc
#            #       I have no idea if/what the wording has changed
#            #       to for other cases
#            if {[string match "Z-Matrix taken from the checkpoint file:" $line] ||
#                [string match "Symbolic Z-Matrix:" $line] ||
...
#            }
#        } else {
#            # currently handles anything prion to G09RevC
#            # Z-matrix given explicitly?      
#            if {[string match "Z-Matrix taken from the checkpoint file:" $line] ||
#                [string match "Symbolic Z-matrix:" $line] ||
#                [string match "Redundant internal coordinates taken from checkpoint file:" $line]} {
 ...
 # endif  
   ...
   if {$simtype=="Geometry optimization" || [string match {CBS-*} $method] || \
	 ...
	 read_gaussian_npacharges $fid
      ...
      # modified to load into current molecule if requested
      if {$intomolid>=0 && [lsearch [molinfo list] $intomolid]>=0} {
         set newmolid [reload_cartesians_as_pdb $tmppdb $molid]
      } else {
...
proc ::QMtool::read_gaussian_mulliken { fid } {
   ...
   while {![eof $fid]} {
	 ...
	 puts $line;  variable normalterm 1; return 0
      ...
      # Look for the beginning of parameter list
      if {$offset} { incr offset; }
      if {[string match "*charges with hydrogens summed into heavy atoms:" $line]} {
	 incr offset;
      }
...
proc ::QMtool::read_scf_energies { file } {
   ...
   if { [llength $scfenergies]>1 && [winfo exists .qmtool] } { 
      #.qmtool.menubar.analysis entryconfigure 0 -state normal
   }
